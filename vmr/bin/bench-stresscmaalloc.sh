...
BUILD_DIR=/usr/src/bench-stresscmaalloc-test
RESULT_DIR=/root/vmregressbench-`uname -r`/cmaalloc-heavy
EXTRA=
...
usage() {
  echo "bench-stresscmaalloc.sh (c) Mel Gorman 2005"
  ...
  echo
  echo "Usage: bench-stresscmaalloc.sh [options]"
...
ARGS=`getopt -o hf:k:t:b:r:e:s:c:ozv: --long help,freemem:,kernels:,tar:,build:,result:,extra:,order:,count:,oprofile,highmem,vmr:,ms-delay:,mb-per-sec:,percent:,gfp-flags: -n bench-stresshcmaalloc.sh -- "$@"`
...
echo "Allocsize:         $ALLOCSIZE"
echo "Allocnum:          $ALLOCNUM"

cat $SHELLPACK_STAP/cmaalloc.stp | sed \
	-e "s/define PARAM_MSDELAY.*/define PARAM_MSDELAY $MS_DELAY/" \
	-e "s/define PARAM_SIZE_ALLOC.*/define PARAM_SIZE_ALLOC $ALLOCSIZE/" \
	-e "s/define PARAM_NUM_ALLOC.*/define PARAM_NUM_ALLOC $ALLOCNUM/" > /tmp/cmaalloc.stp
...
echo Buddyinfo at start of cmaalloc test >> $RESULTS
echo ------------------------------------ >> $RESULTS
cat /proc/buddyinfo >> $RESULTS
echo >> $RESULTS

STARTALLOC=`date +%s`
stap -g /tmp/cmaalloc.stp | tee /tmp/cmaalloc.out
ENDALLOC=`date +%s`
sleep 5
...
echo ---------------------------------------- >> $RESULTS
cat /tmp/cmaalloc.out >> $RESULTS
grep -A 15 "Test completed" /tmp/cmaalloc.out
...
echo >> $RESULTS
echo Buddyinfo at end of cmaalloc test >> $RESULTS
echo --------------------------------- >> $RESULTS
cat /proc/buddyinfo >> $RESULTS
echo >> $RESULTS
...
echo Rerunning cmaalloc test at rest
echo >> $RESULTS
